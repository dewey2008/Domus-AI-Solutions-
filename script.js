...
const devices = {
    ...
    "cameras": "1️⃣ Power on Camera.\n2️⃣ wait for White light to start blinking.\n3️⃣ Connect through mobile app.\nIf these steps fail, Connect to an 'Agent'."
};
...
if(chatNowBtnContact){
    chatNowBtnContact.addEventListener('click', () => {
        ...
        setTimeout(() => {
            ...
            bubble.remove();
            appendMessage("Connected to agent.", 'ai');
            appendMessage("Hello, this is Dewey from DomusAI AI Home Support. How may I help you?", 'ai');
        }, 3000);
    });
};
let cart = [];
  ...
  pricingSwitch.addEventListener('change', () => {
    cards.forEach(card => {
      ...
      if(pricingSwitch.checked) {
        priceElem.innerHTML = `$${yearly}<span>/yr</span>`;
      } else {
        priceElem.innerHTML = `$${monthly}<span>/mo</span>`;
      }
    });
  });
});
